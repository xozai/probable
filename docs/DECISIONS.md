...
| 2026-08-30 | TSV paste is atomic: pasted rows are staged in the grid, nothing persists until every staged row is valid; invalid rows are flagged inline (message per row) and the user fixes or removes them before save. Same all-or-nothing rule as CSV/XLSX import commit (AC2). | Claude | Raised by Honey0 in T-AC3-02; consistent with preview-before-mutation |
